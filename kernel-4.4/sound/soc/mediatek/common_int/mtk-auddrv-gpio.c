...
#if (CONFIG_AUDIO_EXTAMP_AW8733_MODE!=0)
int AudDrv_GPIO_EXTAMP_Select(int bEnable, int mode)
{
	...
	extamp_mode=CONFIG_AUDIO_EXTAMP_AW8733_MODE;
	mutex_lock(&gpio_request_mutex);

 if (bEnable == 1) {
    printk("AudDrv_GPIO_EXTAMP_Select on ,extamp_mode=%d\n",extamp_mode);
	...
	printk("AudDrv_GPIO_EXTAMP_Select off ,extamp_mode=%d\n",extamp_mode);
...
#endif //CONFIG_AUDIO_EXTAMP_AW8733_MODE
