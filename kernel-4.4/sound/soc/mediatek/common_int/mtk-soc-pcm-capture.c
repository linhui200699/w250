...
#ifdef CONFIG_BIDE_DIGIT_MIC  //digit mic i2s0 channel
	...
	if (mPrepareDone == false) {
		...
		*/
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_IN_2) == false) {
			//uint32 MclkDiv0 = 0; //if need MCLK
			...
			//set MCK divider at here
			//EnableALLbySampleRate(m2ndI2SInAttribute.mI2S_SAMPLERATE);
		...
		SetIntfConnection(Soc_Aud_InterCon_Connection,Soc_Aud_AFE_IO_Block_I2S0, Soc_Aud_AFE_IO_Block_MEM_VUL);//I0 I1 to O9 O10
		mPrepareDone = true;
	}
	return 0;
}
