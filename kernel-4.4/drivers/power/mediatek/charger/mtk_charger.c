...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		printk("[charger_routine_thread]Vbat=%d,Ibat=%d,I=%d,VChr=%d,T=%d,Soc=%d,UiSoc=%d,GetType=%d,InfoType=%d,hv:%d pd:%d:%d\n",
			battery_get_bat_voltage(), curr_sign ? bat_current : -1 * bat_current, chg_current, battery_get_vbus(), 
			battery_get_bat_temperature(), battery_get_bat_soc(), battery_get_bat_uisoc(),
			mt_get_charger_type(), info->chr_type, info->enable_hv_charging, info->pd_type, info->pd_reset);
