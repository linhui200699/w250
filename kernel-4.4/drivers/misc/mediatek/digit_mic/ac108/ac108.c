...
//AC108 config
#define AC108_CHANNELS_MAX		6 //8		//range[1, 16]
...
#define AC108_MATCH_DTS_EN		1		//AC108 match method select: 0: i2c_detect, 1:devices tree
...
static int ac108_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	ac108_read(SYSCLK_CTRL, (u8 *)&i, i2c_clt[0]);
...
static int ac108_suspend(struct snd_soc_codec *codec)
{
  ...
  ac108 = dev_get_drvdata(codec->dev);
...
static int ac108_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
  ...
  printk("ac108_i2c_probe,addr=0x%x\n",i2c->addr);
...
#if AC108_MATCH_DTS_EN
	if (!regulator_en) {
		ret = of_property_read_string(np, AC108_REGULATOR_NAME, (const char**)&regulator_name);//
		if (ret) {
			printk(" ac108 regulator name failed \n");
		} else {
			...
			ret = regulator_enable(ac108->vol_supply.vcc3v3);
			if(ret != 0)
				  printk("[AC108] %s: some error happen, fail to enable regulator!\n", __func__);
			regulator_en = 1;
		}
	}
#endif
	...
	if (i2c_id->driver_data < (AC108_CHANNELS_MAX+3)/4) {
		i2c_clt[i2c_id->driver_data] = i2c;
		  ret = snd_soc_register_codec(&i2c->dev, &ac108_soc_codec_driver, (struct snd_soc_dai_driver *)ac108_dai[i2c_id->driver_data], 1);//(struct snd_soc_dai_driver *)
