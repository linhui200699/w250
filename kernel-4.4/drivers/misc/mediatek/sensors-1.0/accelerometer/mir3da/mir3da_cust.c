...
struct i2c_client *handle_client;  //zhou add
...
//zhou add  node show
static ssize_t mir3da_step_show(struct device_driver *ddri, char *buf) 
{
	unsigned char step_temp[2];
	unsigned short steps;
	int ret; 

	mir3da_register_read_continuously(handle_client, NSA_REG_STEPS_MSB, 2, step_temp);

	steps = ((step_temp[0]<<8) + step_temp[1])/2;
	ret = sprintf(buf, "%d\n", steps);
	
    return ret;
}

static ssize_t mir3da_step_clean_store(struct device_driver *ddri, const char *buf, size_t count)
{
	mir3da_register_mask_write(handle_client, NSA_REG_RESET_STEP_CLEAN, 0x80, 0x80);	//step clean . 0x2e mask:0x80
...
#endif 
//zhou add
static DRIVER_ATTR(step,            S_IRUGO,  mir3da_step_show,  NULL);  
static DRIVER_ATTR(step_clean,      S_IRUGO,  NULL,   mir3da_step_clean_store);
/*----------------------------------------------------------------------------*/
