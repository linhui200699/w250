...
#if defined(CONFIG_HAS_EARLYSUSPEND) && defined(CONFIG_EARLYSUSPEND) //add by zhou
...
static int hrm_misc_init(struct hrm_context *cxt)
{
	...
	cxt->mdev.minor = MISC_DYNAMIC_MINOR; //M_HRM_MISC_MINOR  //by zhou modify
...
int hrm_data_report(struct hwm_sensor_data data, int status)
{
	struct hrm_context *cxt = NULL;
	int err = 0;
...
static int hrm_remove(struct platform_device *pdev)
{
  ...
  misc_deregister(&hrm_context_obj->mdev);
	//if (err)
