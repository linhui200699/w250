...
struct hrm_data {
	struct hwm_sensor_data hrm_data;
	int data_updata;
	/* struct mutex lock; */
};
...
struct hrm_context {
	...
	//struct early_suspend early_drv; //by zhou del
...
extern int hrm_data_report(struct hwm_sensor_data data, int status);
