...
#if defined(CONFIG_RGB_LEDS_SUPPORT) // zhou add rgb leds
extern void rgb_leds_interface(unsigned int mode);
#endif

#if defined(CONFIG_LDO_BACKLIGHT_SUPPORT) // zhou add rgb leds
extern void ldo_backlight_interface(unsigned int mode);
#endif

#ifdef CONFIG_MTK_AW9364_DRIVER
...
#if defined(CONFIG_RGB_LEDS_SUPPORT) // zhou add rgb leds
    if(pwm_num==0)
       rgb_leds_interface(2); //red led Off
    else if(pwm_num==1)
       rgb_leds_interface(4); //green led Off  
    else if(pwm_num==2)
       rgb_leds_interface(6); //blue led Off    
#endif		
		break;
...
#if defined(CONFIG_RGB_LEDS_SUPPORT) // zhou add rgb leds
    if(pwm_num==0)
       rgb_leds_interface(1); //red led ON
    else if(pwm_num==1)
       rgb_leds_interface(3); //green led ON   
    else if(pwm_num==2)
       rgb_leds_interface(5); //blue led ON    
#endif		
		break;
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	LEDS_DEBUG("PMIC#%d:%d\n", pmic_type, level);
#ifdef CONFIG_LDO_BACKLIGHT_SUPPORT		
      if(level)
        ldo_backlight_interface(1); //on
      else  
        ldo_backlight_interface(0);
      return 0;
#endif
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
  ...
  printk("[mt_mt65xx_led_set_cust] name=%s,level=%d,data=%ld\n", cust->name, level, cust->data);
	switch (cust->mode) {
...
#ifdef CONFIG_MTK_AW9364_DRIVER
      aw9364_set_backlight_level(level);
      return 0;
#else
		return ((cust_set_brightness) (cust->data)) (level);
#endif
...
void mt_mt65xx_led_work(struct work_struct *work)
{
	...
	printk("mt_mt65xx_led_set_cust 0. level=%d\n", led_data->level );
