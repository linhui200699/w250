...
static void set_pin_mode(int enable)
{
   ...
   if(enable == SPI_PIN_MODE)
	{
	...
	#else	
		mt_set_gpio_mode(LCM_SPI_CS_PIN,LCM_SPI_CS_PIN_M_SPI_CS);
		mt_set_gpio_pull_enable(LCM_SPI_CS_PIN,GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(LCM_SPI_CS_PIN,GPIO_PULL_UP);

		mt_set_gpio_mode(LCM_SPI_SCK_PIN,LCM_SPI_SCK_PIN_M_SPI_CK);
		mt_set_gpio_pull_enable(LCM_SPI_SCK_PIN,GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(LCM_SPI_SCK_PIN,GPIO_PULL_DOWN);

		mt_set_gpio_mode(LCM_SPI_MOSI_PIN,LCM_SPI_MOSI_PIN_M_SPI_MOSI);
		mt_set_gpio_pull_enable(LCM_SPI_MOSI_PIN,GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(LCM_SPI_MOSI_PIN,GPIO_PULL_DOWN);
	#endif
	}
	else{
	#ifdef CONFIG_OF
		...
		dts_select_state(STATE_MI_CLR0);
	#else	
		 mt_set_gpio_mode(LCM_SPI_CS_PIN,LCM_SPI_CS_PIN_M_GPIO);
		 mt_set_gpio_dir(LCM_SPI_CS_PIN,GPIO_DIR_OUT);
		 mt_set_gpio_pull_enable(LCM_SPI_CS_PIN,GPIO_PULL_DISABLE);

		 mt_set_gpio_mode(LCM_SPI_SCK_PIN,LCM_SPI_SCK_PIN_M_GPIO);
		 mt_set_gpio_dir(LCM_SPI_SCK_PIN,GPIO_DIR_OUT);
		 mt_set_gpio_pull_enable(LCM_SPI_SCK_PIN,GPIO_PULL_DISABLE);

		 mt_set_gpio_mode(LCM_SPI_MOSI_PIN,LCM_SPI_MOSI_PIN_M_GPIO);
		 mt_set_gpio_dir(LCM_SPI_MOSI_PIN,GPIO_DIR_OUT);
		 mt_set_gpio_pull_enable(LCM_SPI_MOSI_PIN,GPIO_PULL_DISABLE);
	#endif
	}
	
	#ifndef CONFIG_OF
		mt_set_gpio_mode(LCM_SPI_MISO_PIN,LCM_SPI_MISO_PIN_M_GPIO);
		mt_set_gpio_pull_enable(LCM_SPI_MISO_PIN,GPIO_PULL_DISABLE);
		mt_set_gpio_dir(LCM_SPI_MISO_PIN,GPIO_DIR_OUT);
	#endif
...
#ifdef CONFIG_OF
    spi->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,lcm-spi");
