...
#if defined(CONFIG_ENCODER_INPUT_SUPPORT)
...
#if defined(CONFIG_BIDE_WAKEUP_KEY_SUPPORT)
...
extern void ldo_backlight_callback(void);
#endif

#if defined(CONFIG_ENCODER_INPUT_SUPPORT)
static unsigned int value=0;
static unsigned int value_back=1;
static unsigned int g_gpio_pin;
static unsigned int g_read_pin;
static unsigned int g_gpio_headset_deb;
static int g_encoder_irq;
static unsigned int g_encoder_eint_type = IRQ_TYPE_EDGE_FALLING;
static struct  tasklet_struct encoder_tasklet;
static void tasklet_handler(unsigned long data);
static DECLARE_TASKLET(encoder_tasklet, tasklet_handler, 0);
#endif
...
#endif
#if defined(CONFIG_ENCODER_INPUT_SUPPORT)
static const u16 kpd_encoder_keymap[] = {

#if defined(CONFIG_ENCODER_INPUT_DEBUG)	
	KEY_VOLUMEUP, KEY_VOLUMEDOWN,
#else
	KEY_F4, KEY_F5,
#endif
...
#if defined(CONFIG_LDO_BACKLIGHT_SUPPORT) // zhou add leds
     ldo_backlight_callback(); //try open button leds
#endif
...
static void tasklet_handler(unsigned long data)
{
  ...
  if(value_back == value)
  {
	   ...
	   input_report_key(kpd_input_dev, KEY_VOLUMEUP, 0);
#else
	   input_report_key(kpd_input_dev, KEY_F4, 1);
	   input_report_key(kpd_input_dev, KEY_F4, 0);
#endif  
	   input_sync(kpd_input_dev);
	    
	   if(value){
	   		value_back=0;
	   		irq_set_irq_type(g_encoder_irq, IRQ_TYPE_EDGE_RISING);	
	   }else{
	   		value_back=1;  
	   		irq_set_irq_type(g_encoder_irq, IRQ_TYPE_EDGE_FALLING);		
	   }   
	}else{
#if defined(CONFIG_ENCODER_INPUT_DEBUG)	
	   input_report_key(kpd_input_dev, KEY_VOLUMEDOWN, 1);
	   input_report_key(kpd_input_dev, KEY_VOLUMEDOWN, 0);
#else
	   input_report_key(kpd_input_dev, KEY_F5, 1);
	   input_report_key(kpd_input_dev, KEY_F5, 0);
#endif  	
	   input_sync(kpd_input_dev);
	...
	enable_irq(g_encoder_irq); 
}
static irqreturn_t encoder_ap_eint_func(int irq, void *data)
{
  disable_irq_nosync(g_encoder_irq);  
  tasklet_schedule(&encoder_tasklet);
	return IRQ_HANDLED;  
}
int encoder_dts_get(void)
{
	int ret = 0;
	u32 ints1[4] = { 0 };
	struct device_node *node = NULL;
	
  node=of_find_compatible_node(NULL, NULL, "mediatek,encoder");
	if (node) {	
	  g_read_pin = of_get_named_gpio(node, "encoder_read", 0);
	  gpio_request(g_read_pin, "encoder_r");
	  gpio_direction_input(g_read_pin);
	  
		g_gpio_pin = of_get_named_gpio(node, "deb-gpios", 0);
		ret = of_property_read_u32(node, "debounce", &g_gpio_headset_deb);
		if (ret < 0) {
			printk("debounce time not found\n");
			return ret;
		}
		gpio_set_debounce(g_gpio_pin, g_gpio_headset_deb);
		g_encoder_irq = irq_of_parse_and_map(node, 0);
		of_property_read_u32_array(node, "interrupts", ints1, ARRAY_SIZE(ints1));
		g_encoder_eint_type = ints1[1];
		
		printk("[encoder]g_read_pin:%d gpiopin:%d debounce:%d g_encoder_irq:%d g_encoder_eint_type:%d\n",
		        g_read_pin,	g_gpio_pin, g_gpio_headset_deb, g_encoder_irq, g_encoder_eint_type);
		
		ret = request_irq(g_encoder_irq, encoder_ap_eint_func, IRQF_TRIGGER_NONE, "encoder-eint", NULL);
		if (ret != 0) {
			printk("[encoder]EINT IRQ LINE NOT AVAILABLE\n");
			return -1;
		} else {
			printk("[encoder]encoder set EINT finished, encoder_irq=%d, headsetdebounce=%d\n",  g_encoder_irq, g_gpio_headset_deb);
		}
	} else {
		printk("[encoder]%s can't find compatible node\n", __func__);
		return -1;
	}
	
	printk("[encoder]encoder_dts_get success!\n");
	return 0;
}
#endif
static int kpd_pdrv_probe(struct platform_device *pdev)
...
#if defined(CONFIG_ENCODER_INPUT_SUPPORT)
	for (i = 0; i < ARRAY_SIZE(kpd_encoder_keymap); i++)
		__set_bit(kpd_encoder_keymap[i], kpd_input_dev->keybit);
#endif
