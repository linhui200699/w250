...
#ifdef REGULATOR_VCC2V8
struct regulator *vcc2v8;
#endif
#ifdef GSL_GESTURE
typedef enum{
	GE_DISABLE = 0,
...
static int gsl_test_i2c(struct i2c_client *client)
{
	...
	if(err>0){
	  	printk("[tp-gsl] i2c read success. 0xfc = 0x%02x%02x%02x%02x\n",	buf[3],buf[2],buf[1],buf[0]);
	}else{
	    printk("[tp-gsl] i2c read 0xfc error!");			
	}
...
static void gsl_sw_init(struct i2c_client *client)
{
  ...
  printk("[gsl_sw_init] step 1.\n");
	temp = gsl_test_i2c(client);
	if(temp<0){
	  printk("[gsl_sw_init] gsl_test_i2c error!\n");
		gsl_sw_flag = 0;
		return;
	}
  printk("[gsl_sw_init] step 2.\n");
	gsl_clear_reg(client);
	gsl_reset_core(client);
	printk("[gsl_sw_init] step 3.\n");
	...
	gsl_sw_flag=0;
	printk("[gsl_sw_init] step 5.\n");
	write_buf[3] = 0;
	write_buf[2] = 0;
	write_buf[1] = 0;
	write_buf[0] = 0xb6;
	gsl_write_interface(client, 0xf0, write_buf, 4);
	printk("[gsl_sw_init] step 6.\n");
	printk("[gsl_sw_init] done.\n");
	
	//gsl_read_interface(client, 0x7c, read_buf, 4);
	//printk("[gsl_sw_init] step 7.\n");
	//printk("[gsl_sw_init][%s] addr = (b6,7c); read_buf = %02x%02x%02x%02x\n", __func__, read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
}
...
#ifdef REGULATOR_VCC2V8
	int ret = 0;
	tpd->reg = regulator_get(NULL, REGULATOR_NAME);
  ret = regulator_set_voltage(tpd->reg, 2800000, 2800000);    /* set 2.8v */
  if(ret)
      printk("[gsl2038] regulator_set_voltage() failed!\n");
  ret = regulator_enable(tpd->reg);   /* enable regulator */
  if (ret)
      printk("[gsl2038] regulator_enable() failed!\n");
#endif       
  tpd_gpio_output(GTP_RST_PORT, 1);
  msleep(20);
	tpd_gpio_output(GTP_RST_PORT, 0);
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(touch_irq, (irq_handler_t)tpd_eint_interrupt_handler,
					IRQF_TRIGGER_RISING , "TOUCH_PANEL-eint", NULL); //IRQF_TRIGGER_FALLING
			if (ret > 0)
	...
	printk("[tpd_irq_registration] done.\n");
	return 0;
}
...
static int  gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  printk("[gsl_probe] gsl_sw_init done.\n");

  tpd_irq_registration();
  printk("[gsl_probe] tpd_irq_registration done.\n");
  
	thread = kthread_run(touch_event_handler, 0, TPD_DEVICE);
	if (IS_ERR(thread)) {
	    printk("[tpd-gsl][%s] kthread_run error!\n",__func__);
	}
	printk("[gsl_probe] kthread_run done.\n");
...
err_malloc:
	  if(ddata)
	  {
		...
		{
			printk("[gsl2038] tpd_probe regulator_disable() failed!\n");
		}
